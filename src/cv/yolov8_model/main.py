...
_FACE_DIR = os.path.join(_PROJECT_DIR, "face_recognition")
names_file_path = os.path.join(_FACE_DIR, "names.json")
# Check for GPU availability
device = "cuda" if torch.cuda.is_available() else "cpu"
print(f"Using device: {device}")

# Load YOLOv8 model
model = YOLO(os.path.join(_SCRIPT_DIR, "yolov8n.pt")).to(device)

# Load face recognition model
face_recognizer = cv2.face.LBPHFaceRecognizer_create()
face_model_path = os.path.join(_FACE_DIR, "trainer.yml")
...
if os.path.exists(face_model_path):
    ...
    os.system(f'python3 "{os.path.join(_FACE_DIR, "src", "face_taker.py")}"')
    os.system(f'python3 "{os.path.join(_FACE_DIR, "src", "face_trainer.py")}"')
    face_recognizer.read(face_model_path)
